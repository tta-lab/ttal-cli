...
func (_q *TagQuery) QueryAgents() *AgentQuery {
...
func (_q *TagQuery) WithAgents(opts ...func(*AgentQuery)) *TagQuery {
	query := (&AgentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TagQuery) loadAgents(ctx context.Context, query *AgentQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Agent)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Tag)
	nids := make(map[int]map[*Tag]struct{})
	for i, node := range nodes {
