	...
	for _, entry := range entries {
		...
		if !dryRun {
			if err := os.Remove(path); err != nil {
				return nil, fmt.Errorf("removing stale Codex agent %s: %w", path, err)
			}
		}
	}

	if !dryRun && len(removed) > 0 {
		if err := cleanCodexConfigEntries(configPath, removedNames); err != nil {
			return nil, err
		}
	}
...
// cleanCodexConfigEntries removes agent entries from config.toml for agents whose
// .toml files were already verified as ttal-managed and removed. removedNames contains
// the names of agents that were cleaned up.
func cleanCodexConfigEntries(configPath string, removedNames map[string]bool) error {
	cfg, err := loadOrInitConfig(configPath)
	if err != nil {
		return err
	}

	agentsSection, ok := cfg["agents"].(map[string]interface{})
	if !ok {
		return nil
	}
	...
	changed := false
	for name := range agentsSection {
		if _, isMap := agentsSection[name].(map[string]interface{}); !isMap {
			continue
		}
