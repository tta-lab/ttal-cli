...
func TestDBPathForTeam(t *testing.T) {
	defDataDir := defaultDataDir()
	...
	tests := []struct {
		...
		{
			name: "team with explicit DBPath",
			cfg: &Config{
				Teams: map[string]TeamConfig{
					"guion": {DBPath: "~/custom/guion.db"},
				},
			},
			team:     "guion",
			wantPath: expandHome("~/custom/guion.db"),
		},
		{
			name: "team without DBPath uses convention",
			cfg: &Config{
				Teams: map[string]TeamConfig{
					"guion": {},
				},
			...
			team:     "guion",
			wantPath: defDataDir + "/guion/ttal.db",
		},
		{
			name: "default team uses root data dir",
			cfg: &Config{
				Teams: map[string]TeamConfig{
					DefaultTeamName: {},
				},
			...
			wantPath: defDataDir + "/ttal.db",
		},
		{
			name: "team not found returns error",
			cfg: &Config{
				Teams: map[string]TeamConfig{
					"guion": {},
				},
			...
			team:    "nonexistent",
			wantErr: `team "nonexistent" not found in config`,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := tt.cfg.dbPathForTeam(tt.team)
			if tt.wantErr != "" {
				if err == nil {
			...
			if got != tt.wantPath {
				t.Errorf("dbPathForTeam(%q) = %q, want %q", tt.team, got, tt.wantPath)
			}
		})
	}
}
