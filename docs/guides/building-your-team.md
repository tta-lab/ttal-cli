...
Kestrel manages your tasks, responds to Telegram messages, and spawns workers when you run `ttal go`. Each worker is isolated in its own git worktree within a named tmux window in the owner manager session (ttal-default-{owner}:{agent}).
