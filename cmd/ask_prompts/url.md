...
Use the `read_url` tool to retrieve the page content:
...
```
read_url(url="{rawURL}", section="<id>")
```
...
If the page doesn't contain the answer, use `search_web` to find related pages or check official docs.
...
- Always use `read_url` — never try to infer content you haven't fetched
